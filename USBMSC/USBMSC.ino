...
static const uint32_t DISK_SECTOR_COUNT = 2 * 7.25 * 1024 * 1024; // 8KB is the smallest size that windows allow to mount
...
static int32_t onWrite(uint32_t lba, uint32_t offset, uint8_t* buffer, uint32_t bufsize) {
  ...
  int x = SD.writeRAW((uint8_t*)buffer, lba, ceil(bufsize/SD.sectorSize()));  
  //int x = SD.writeRAW((uint8_t*)buffer, lba, 1);  
  return bufsize;
}
...
static int32_t onRead(uint32_t lba, uint32_t offset, void* buffer, uint32_t bufsize) {
  ...
  //int x = SD.readRAW((uint8_t*)buffer, lba);
  int x = SD.readRAW((uint8_t*)buffer, lba, ceil(bufsize/SD.sectorSize()));
  //if(bufsize > 512)
    //return 512;
  return bufsize;    
}
...
void setup() {
  ...
  uint8_t buffers[512]={255};
  SD.readRAW(buffers, 0);
  for (int i = 0; i < 512; i++)
  {
    if (i % 16 == 0) HWSerial.println();
    HWSerial.printf("[%02X]", buffers[i]);
  }

  HWSerial.println();
  HWSerial.println(SD.sectorSize());
  HWSerial.println();
  SD.readRAW(buffers, 8192);
  for (int i = 0; i < 512; i++)
  {
