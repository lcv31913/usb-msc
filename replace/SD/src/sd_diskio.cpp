...
bool sd_read_raw(uint8_t pdrv, uint8_t* buffer, DWORD sector, uint32_t number)
{
    return ff_sd_read(pdrv, buffer, sector, number) == ESP_OK;
}

bool sd_write_raw(uint8_t pdrv, uint8_t* buffer, DWORD sector, uint32_t number)
{
    return ff_sd_write(pdrv, buffer, sector, number) == ESP_OK;
}
/*
 * Public methods
 * */
